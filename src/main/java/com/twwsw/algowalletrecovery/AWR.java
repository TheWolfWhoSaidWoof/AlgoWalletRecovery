package com.twwsw.algowalletrecovery;
...
public class AWR {
    ...
    public static void main(String[] args) {
        if (args.length!=1)
        {
            System.err.println("Syntax: java <jar name> \"25 word mnemonic\"");
            System.err.println("The mnemonic must be quoted");

            System.exit(-1);
        }
        //lowercase it
        String[] start=args[0].toLowerCase(Locale.ROOT).split(" ");
        //validate all 25 words are BIP39;
        int n=0;
        for(String word:start)
        {
            if(Arrays.stream(ENGLISH_WORD_LIST).noneMatch(word::equals))
            {
                System.err.println("Word "+(n+1)+" "+start[n]+" is not a bip39 word. Check here https://github.com/bitcoin/bips/blob/master/bip-0039/english.txt");
                start[n]="==>"+start[n]+"<==";
                System.err.println(String.join(" ",start));
                System.exit(-1);
            }
            n++;
        }
        String[] now=new String[25];
        int count=0;

        //fold some space
        //loop over each word in mnemonic
        for (int word=0;word< start.length;word++)
        {
            //copy start to now
            int i=0;
            for (String s : start) {
            ...
            //try each bip39 word in place of the existing word
            for (String s : ENGLISH_WORD_LIST) {
                now[word] = s;
                Account acc;
                try {
                    acc = new Account(String.join(" ", now));
                    //no exception means valid account. Output it
                    System.out.println("valid " + (++count) + ":" + String.join(" ", now));
                } catch (GeneralSecurityException e) {
                    //ignore . these are invalid accounts
                }
            }
    ...
    //This is the English language word list. Replace with your language's word list if required
    // alternatives are here https://github.com/bitcoin/bips/tree/master/bip-0039
